...
## 2-A. 보고 원칙 (실제 사고로 만들어진 규칙 — 반드시 지킬 것)

**사용자는 코드를 읽지 못한다. 내 보고를 그대로 믿고 돈과 시간을 쓴다.**
2026-07 세션에서 측정 없이 단정한 세 번의 오답("스크래핑이 병목", "AI가 병목", "이 코드는 중복이니 삭제")으로
사용자의 비용과 시간이 낭비되었다. 그래서 아래 5개는 예외 없이 지킨다.

1. **측정 전에 원인을 단정하지 않는다.** "아마 ~때문일 것"으로 시작하는 결론 금지. 로그·실측 수치를 먼저 확보한다.
2. **1회 측정으로 결론 내지 않는다.** 네트워크·외부 API는 편차가 크다. 최소 3~5회 측정 후 평균으로 말한다.
3. **코드를 지우자고 하기 전에 "왜 이게 있는지"를 먼저 확인한다.** 이유를 설명하지 못하면 삭제 제안 자체를 하지 않는다.
4. **사용자가 지적하기 전에 내가 먼저 정정한다.** 앞선 발언이 틀린 걸 알게 되면 즉시 정정 보고한다.
5. **모르면 "모른다"고 말한다.** 추정치는 반드시 "추정"이라고 명시하고, 확인 방법을 함께 제시한다.

---

## 2-B. 절대 건드리면 안 되는 것 (사고 방지)

- **`src/lib/autoMarkerScan.js` 3단계의 `getAll` 존재 확인 로직을 제거하지 말 것.**
  이건 "이미 있는 마커인지"를 판별하는 안전장치다. 지우면 기존 마커를 신규로 착각해 **AI를 다시 호출하고
  관리자가 손수 수정한 마커(현재 181개)를 덮어쓴다.** 중복처럼 보여도 중복이 아니다.
- **자동화 브라우저로 사이트를 테스트할 때 영상 카드를 클릭하지 말 것.**
  헤드리스 환경에서는 iframe 재생이 실패해 `report-error` 가 호출되고, **실제 운영 마커가 자동 비활성화된다.**
  (실제로 마커 1개가 이 방식으로 손상되어 복구했다.) 화면 확인은 `read_page`/스크린샷으로만.
- **`.env.local` / 서비스 계정 JSON / API 키를 채팅에 붙여넣지 말 것.**
- **YouTube API 키를 여러 개로 쪼개 할당량을 늘리지 말 것** (약관 위반). 백업 키는 `NODE_ENV` 게이트로 개발 전용,
  Vercel 환경변수에 넣지 않는다.

---

## 2-C. Firestore 비용 규칙 (반복 사고 재발 방지)

2026-07-13~25 사이 Firestore 읽기가 무료 한도(5만/일)를 반복 초과했다(최고 356K/일). 매번 **일부만 고쳐서 재발**했다.

- **공개 화면의 마커 조회는 반드시 `src/lib/markerIndex.js`(청크 인덱스)를 거친다.**
  `marker_index/chunk_N` 문서 몇 개만 읽으므로 **마커가 3만 개가 되어도 읽기 수가 늘지 않는다.**
  컬렉션 전체 스캔(`collection().get()`) 코드를 새로 추가하지 않는다.
- **새 기능을 추가하면 "이 코드가 마커 수에 비례해 읽는가?"를 반드시 자문한다.** 비례하면 인덱스를 쓰도록 고친다.
- **부분 수정 금지 — 같은 유형의 코드를 전수조사한다.** 한 파일만 고치고 "해결했다"고 보고하지 않는다.
- **`unstable_cache` 는 Vercel 서버리스 인스턴스 간에 공유되지 않는다.** 전역 캐시가 필요하면 Firestore 스냅샷을 쓴다.
- **집계용 맵 필드의 키는 반드시 고정된 소수의 분류값으로 축약한다.** URL·리퍼러 원문을 키로 쓰면 문서가
  1MB 한도를 넘겨 터진다. (`src/app/api/track/route.js` 의 `classify*` 함수 참고)
- **한도 리셋 기준은 태평양 자정**(한국·태국 오후 2~3시)이다. UTC 자정 아니다.
- **지역이 특정되지 않은 마커(`continent` 없음)는 공개 화면에서 제외**한다 — `src/lib/hasLocation.js` 를
  **5개 파일 전부**(`markerIndex`, `queryPublicMarkers`, `getAutoMarkers`, `getPublicMarkers`, `getPublicMarkerById`)에 적용해야 한다.

📄 상세 경위·수치·파일별 변경 내역은 **`HANDOVER-2026-07-28.md`** 참고.
