...
//
// ⚠️ 2026-07-29 인증 추가: 이 라우트는 markers 컬렉션 "전체"를 매 호출마다 캐시 없이
//    스캔한다(관리자 폼 자동완성 저빈도 호출을 가정하고 설계됨). 그런데 인증이 없고
//    robots.txt 의 "Disallow: /api" 가 오히려 /api 경로 존재를 크롤러/스캐너에 알려주는
//    역할을 해, 봇이 이 URL 을 반복 호출하면 호출 1회당 컬렉션 전체 읽기(실측 324개)가
//    발생한다. Firestore 읽기 초과 조사 중 발견(HANDOVER-2026-07-28.md §8 에도
//    "잠재 위험"으로 이미 기록돼 있었음). → 다른 관리자 전용 라우트와 동일하게
//    verifyAdminRequest 로 보호한다. 도시 자동완성은 관리자 폼(MarkerForm)에서만
//    쓰이므로 기능 변화 없음.
  ...
  try {
    ...
    if (!authResult.valid) {
      return Response.json(
        { ok: false, error: authResult.error || "로그인이 필요합니다", cities: [] },
        { status: 401 }
      );
    }
